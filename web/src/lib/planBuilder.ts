...
export function metaFromPlanExport(curriculum: CourseRecord[], config: Record<string, unknown>): MetaResponse {
  ...
  return {
    ...
    initial_state: (config.initial_state as MetaResponse["initial_state"]) ?? { occupancy: {} },
    admission_terms: (config.admission_terms as string[] | undefined) ?? ["Fall"],
    admission_sizes: (config.admission_sizes as Record<string, number> | undefined) ?? {},
    ...
    mandatory_terms: (config.mandatory_terms as string[] | undefined) ?? ["Fall", "Spring"],
...
export function composePlanPayload(
  ...
  const config: Record<string, unknown> = {
    ...
    initial_state: { occupancy: state.initialOccupancy },
    admission_terms: state.admissionTerms,
    admission_sizes: state.admissionSizes,
