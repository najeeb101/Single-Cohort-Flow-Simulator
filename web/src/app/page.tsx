...
import type { MetaResponse, SimulateResponse } from "@/types/simulation";
...
export default function Home() {
  ...
  // Fixed once from the baseline load, like frontend/app.js::buildLiveControls() — the
  // slider set must NOT be recomputed from live results, or a course that a slider drags
  // out of the top-3 capacity list disappears from `topCapacityCourses` on the next
  // render while LiveWhatIfPanel's slider state still has a value keyed on it, crashing.
  ...
  return (
    <main className="mx-auto max-w-6xl px-7 pb-16">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[12.5px] text-[#c9a6b2]">
              Multi-cohort, shared-seat university model — dashboard (slice 1, web/)
            </p>
          </div>
        </div>
      </header>
      ...
      <p className="py-6 text-xs text-muted">
        The animated curriculum graph and figures are not yet ported to this app — see{" "}
        <code className="rounded bg-black/35 px-1.5 py-0.5">frontend/</code> for the full
        existing dashboard, or{" "}
