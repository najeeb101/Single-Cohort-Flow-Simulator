  ...
  return (
    <main className="mx-auto w-full max-w-[1600px] px-7 pb-16">
      <header className="border-b border-border py-5">
        <h1 className="text-[19px] font-bold tracking-tight">Semester Checkpoint</h1>
        <p className="mt-0.5 max-w-2xl text-sm text-muted">
          A turn-based walkthrough of the active plan: advance one semester at a time and adjust
          capacity, pass rates, occupancy, and next intake between steps — future decisions only,
          never the courses already run. This never touches the baseline dashboard.
        </p>
      </header>
  ...
  return (
    <div className="mx-auto mt-10 flex max-w-lg flex-col items-center gap-4 rounded-2xl border border-border bg-surface px-6 py-10 text-center">
      <p className="text-sm text-muted">
        No walkthrough is in progress. Starting one begins a fresh, independent re-run of the
        active plan from term one — the always-on baseline dashboard elsewhere is unaffected.
      </p>
      <button
        type="button"
        onClick={onStart}
