# Product Roadmap: From Research Tool to ACIP
...
was **deleted** in the Phase 1 re-prioritization documented in `docs/input_system_history.md`
...
**Problem.** Pass rates, dropout hazards, load caps are config *guesses*. Which of them is actually
the binding lever on graduation rate is itself config-dependent — a sensitivity sweep run against
one configuration found gateway-section counts barely moved the rate while dropout hazard moved it
more; re-running the same sweep after the optional-term model was added reversed that ranking. So
"which knob matters" is a question to ask fresh against the *current* configuration, not something
this project can claim to have settled once.
**Action.** Extend the `scripts/size_sections.py` calibration pattern so course pass rates, dropout parameters, and section demand are **fit from the partner institution's history**. The engine stays the same; its inputs become empirical. Done for pass rates + dropout hazard (above); section demand was already covered by `scripts/size_sections.py`. `normal_load_ch` now has a measured sanity check too (`fit_load_cap` — empirical p95 per-student-term credit load from history, same demand-percentile idea as section sizing), reported but not auto-applied since it's a registration policy, not an observed rate. Still open: `probation_load_ch` and the dropout front-loading multiplier remain assumed — lower priority since `probation_load_ch` only applies to an already-small probation subpopulation.
...
- ⏪ Built, then deleted: §2.2 measured calibration (fit pass rates / dropout hazards from
  ...
  `docs/input_system_history.md` §1.1 to ship the Scenario Builder faster. See §2.2.
...
planned in this doc — it came out of `docs/input_system_history.md`'s own re-scoping (now
...
**Status: done, then superseded by something bigger.** `src/api.py` exposes `POST /simulate` (scenario overrides in, the same `flow_timeline` contract back out) and `GET /meta` (curriculum graph + slider-relevant config). The original control panel, `web/src/components/LiveWhatIfPanel.tsx` (2 sliders), was deleted and replaced by the multi-tab **Scenario Builder** (`web/src/components/scenario-builder/` — Capacity / Pass Rates & Dropout / Admissions / Registration Policy, each with a Simple/Advanced toggle) per `docs/input_system_history.md` §1.3, which covers far more of the engine's levers than this sketch originally scoped. Rest of this section kept as the original design sketch.
