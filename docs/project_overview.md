...
- *How much does the shared seat pool's behavior vary year to year just from cohort-to-cohort
  ...
  average.
- *What should the curriculum committee actually do about it, and how many seats does that take?*
  The Advisor panel turns a run's own numbers into prioritized plain-language recommendations;
  Auto-fill goes a step further and searches for the smallest set of capacity additions that meets
  the committee's own admission targets, instead of leaving "how many more seats" as a guess.
- *For one specific (synthetic) student, what actually happened to them, term by term, and why?*
  The Student Trace page answers this directly — pick a student by profile (e.g. "someone who was
  censored," "someone who was ever on probation") and see their exact path: which courses they
  passed or failed, which term they got stuck and by which of the four signals, and how their GPA
  moved. It's the individual-level complement to every aggregate question above.
...
8. **It is not a predictive tool for any named, real individual student.** This is a
   ...
   student will pass their next course. The Student Trace page (§7) shows the exact path of one
   *synthetic* student already generated by a completed run — a diagnostic lens on the model's own
   output, not a forecast about anyone real.
