...
**Mentor / Supervisor:** [Insert supervisor name]  
**Internship Period:** [Insert internship period]  
**Institution / Organization:** Qatar University, Department of Computer Science  
...
I would like to express my sincere gratitude to my supervisors, instructors, and colleagues for their guidance and support throughout this internship. Their feedback helped me improve both the technical quality of the project and the way I explained the purpose of the system.
...
- Improve my ability to work independently on a long-running software project.
...
### 2.4 Connection to the Organization's Mission

The project connects to Qatar University's mission by supporting academic quality, data-informed planning, and student success. By making curriculum bottlenecks visible, the simulator can help academic planners discuss possible improvements to course capacity, prerequisite structure, offering patterns, and admission size. The system does not replace academic judgment, but it provides structured evidence that can support better decisions.
...
The first stage of the internship focused on understanding the curriculum and designing the core simulation approach. I reviewed the Computer Science study plan, prerequisite structure, and the main assumptions needed to represent student movement through the program.

I then built the core Python simulation engine. The engine uses a discrete-term loop where each student attempts to register for courses, receives seats based on capacity and priority, and then either passes or fails enrolled courses. Each student has their own academic state, including completed courses, credit hours, GPA, probation status, and graduation status.
...
### 4.2 Week 2: Backend, Database, and Dashboard Development

After the first version of the engine was working, I moved the project from a script-based simulation into a full-stack application. I worked on a FastAPI backend that allowed the simulation to be run through HTTP endpoints. I also contributed to the SQLAlchemy and SQLite persistence model so that courses, plans, configurations, scenarios, and runs could be represented in a structured way.

On the frontend, I worked with a Next.js and React dashboard. The dashboard presents the simulation output through visual pages such as the main dashboard, bottleneck analysis, figures, settings, plans, and advisor views. This made the simulator easier to use because users no longer needed to read raw output files or inspect code to understand the results.

![Figure 1: Main dashboard showing simulation results, headline metrics, and the curriculum roadmap](images/report_dashboard_overview.png)

Figure 1 shows the main dashboard, where a user reviews headline simulation results next to a program-roadmap layout of the curriculum. It illustrates how the system turns complex academic data into an accessible visual summary for planning and analysis, and it is the page every planning session starts from.

### 4.3 Week 3: Bottleneck Analysis and Semester Checkpoint Mode

The next stage focused on making the results more meaningful. Instead of reporting only graduation rate or average time to graduate, the simulator records four separate bottleneck signals:

- Failure blocks, when students attempt a course and fail.
- Capacity blocks, when students are eligible but cannot get a seat.
- Offering blocks, when students are eligible but the course is not offered that term.
- Prerequisite blocks, when students are waiting for an upstream course.

This separation was one of the most important parts of the project. It prevents different problems from being treated as the same issue. For example, a course with many failures may need academic support, while a course with many capacity blocks may need additional seats. A course with many prerequisite blocks may not be the real problem itself; it may simply be downstream of another difficult gateway course.

![Figure 2: Bottlenecks page showing ranked failure, capacity, and offering issues with capacity recommendations](images/report_bottlenecks_analysis.png)

Figure 2 presents the bottlenecks analysis page, where the system ranks the main causes of delay, failure, capacity pressure, and offering limitations, separately, and suggests a capacity target for each flagged course.

I also built a semester checkpoint mode, which lets an academic planner advance the simulation one term at a time (including the optional Summer/Winter terms), pause to edit future-facing settings such as capacity or intake size, and continue from exactly where the run left off, rather than editing and re-running the whole twelve-semester horizon from scratch. Because the underlying engine can save and restore its full internal state, a session can also step back to any earlier term to preview it before deciding whether to commit to that point and continue forward from there. This matches how a real planning conversation actually happens: a decision may need to be revisited as new information appears, not committed once and left alone.

![Figure 3: Analytics page showing university population and per-cohort flow charts](images/report_analytics_figures.png)

Figure 3 shows the analytics view, where university enrollment trends and per-cohort flow patterns are displayed. It helps illustrate how the simulator captures changes over time and how different student groups move through the curriculum.

### 4.4 Week 4: Refinement, Decision-Support Tools, Testing, and Documentation

In the final stage, I worked on refining the system and improving its clarity. Some earlier features were simplified so that the application focused more directly on the research question: what causes delay, and which structural changes may reduce it?

I also built two decision-support features on top of a completed run. The first is a rules-based advisor panel that reads a run's headline metrics and top bottlenecks and turns them into a prioritized, plain-language list of recommendations, with no external service involved. The second is an optional chat interface, grounded in the same run's numbers and the full active curriculum, that can answer detailed questions about a specific course and, when it recommends a concrete numeric change, propose it as a card the planner can first test in a sandboxed simulation and only then apply; the model itself never writes to the plan directly. Alongside these, I built a bounded capacity-planning solver that searches for the smallest set of capacity increases needed to bring seat denials down to a target level, and a multi-plan model so that more than one curriculum-and-configuration combination can be stored, imported, and compared without one overwriting the other.

![Figure 4: Advisor page showing recommendations and what-if analysis tools](images/report_advisor_recommendations.png)

Figure 4 illustrates the advisor page, which combines the rules-based recommendations with the grounded chat interface and its testable proposals. It shows that the project is not only about generating data, but also about supporting decision-making through practical, checkable suggestions rather than a black-box answer.

I also improved testing and documentation. The project includes tests for determinism, prerequisite logic, graduation behavior, capacity allocation, optional terms, API behavior, database behavior, checkpoint stepping, and other system features. This was important because small changes in a simulation can affect many results, so automated tests helped protect the behavior of the system.

The dashboard and analytics figures became the main way to communicate the project. They show university enrollment, cohort flow, graduation distribution, seat utilization, bottlenecks, and the prerequisite network.

![Figure 5: Settings page showing curriculum and configuration editing tools](images/report_settings_configuration.png)

Figure 5 shows the settings page, where curriculum and configuration values can be adjusted. It highlights the flexibility of the platform and shows that the simulation can be adapted to different planning scenarios and institutional policies.
...
As the project grew, it became clear that adding more features did not always make the system better. Some features increased complexity without directly supporting the main research question. I learned to simplify the application and focus on the capabilities that mattered most: simulation, bottleneck analysis, scenario comparison, configuration, and clear visualization.

This was an important professional lesson. Good software is not only about adding functionality; it is also about choosing the right scope and keeping the system understandable.
...
The internship provided a meaningful opportunity to build a complete software system around an academic planning problem. The Single-Cohort Flow Simulator demonstrates how simulation, backend development, database modeling, analytics, and frontend visualization can be combined into a practical decision-support tool.
...
1. Main dashboard
2. Bottleneck analysis page
3. Analytics figures page
4. Advisor recommendations page
5. Settings and configuration page
