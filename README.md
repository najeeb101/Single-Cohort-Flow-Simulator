...
└── reports/
    ...
    └── flow_timeline.json            # frontend contract for the scenario that feeds the dashboard (frontend/ fetches it directly, no copy)
```
...
85 tests cover determinism, the 120-credit-hour reconciliation, graduation detection,
...
graduate distribution, seat-utilization heatmap) as React/SVG — talking to the API
directly, it does not read `outputs/`. The prerequisite-network diagram isn't ported
(the animated curriculum graph already covers that view interactively); `frontend/`
remains the fallback full dashboard for that one figure.
...
web/         Next.js/TypeScript dashboard (Phase 2) — talks to src/api.py; figures ported,
             prerequisite-network diagram excluded (animated graph covers it)
