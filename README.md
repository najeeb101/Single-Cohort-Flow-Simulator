...
admissions recommendation, live what-if sliders, and the animated semester-by-semester
curriculum graph (playback controls, narrative panel, per-cohort stage/flow side panel)
— talking to the API directly, it does not read `outputs/`. The static figures aren't
ported yet; `frontend/` remains the fallback full dashboard until that's done.
...
web/         Next.js/TypeScript dashboard (Phase 2) — talks to src/api.py; figures not ported yet
