...
### Run the Next.js dashboard (optional, Phase 2 slice 1)
...
admissions recommendation, and live what-if sliders, talking to the API directly — it
does not read `outputs/`. The animated curriculum graph and the static figures aren't
ported yet; `frontend/` remains the full dashboard until a later slice closes that gap.
...
web/         Next.js/TypeScript dashboard (Phase 2 slice 1) — talks to src/api.py, no animated graph yet
